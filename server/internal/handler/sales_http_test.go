...
func newSalesServerDB(t *testing.T) (*echo.Echo, *sql.DB) {
	...
	g.GET("/sales/groups", h.Sales.Groups)
	g.GET("/sales/groups/new", h.Sales.GroupNew)
	g.POST("/sales/groups", h.Sales.GroupCreate)
	g.GET("/sales/groups/preview.json", h.Sales.GroupPreview)
	g.GET("/sales/groups/:id", h.Sales.GroupShow)
	g.GET("/sales/groups/:id/edit", h.Sales.GroupEdit)
	g.POST("/sales/groups/:id", h.Sales.GroupUpdate)
	g.POST("/sales/groups/:id/members", h.Sales.GroupAddMembers)
	g.POST("/sales/groups/:id/members/:sid/remove", h.Sales.GroupRemoveMember)
	g.POST("/sales/groups/:id/close", h.Sales.GroupClose)
	g.GET("/sales/groups/:id/search-projects", h.Sales.GroupSearchProjects)
	g.POST("/sales/bulk-biz", h.Sales.BulkBizType)
	g.POST("/sales/bulk-budget", h.Sales.BulkBudgetStatus)
